class DistanceResolver
  ...
  def start_coordinates(trip)
    ...
    start_location.nil? ? 0: [start_location.latitude,start_location.longitude]
  end

  def destination_coordinates(trip)
    destination_location = Location.find_by(address: trip.destination_address)
    destination_location.nil? ? 0 : [destination_location.latitude,destination_location.longitude]
  end
end
